...
def parse_task_dictionary(db_name, task_dictionary):
    """ 
        Return a dictionary with task information. 返回一个包含任务信息的字典
        Additionally we return a dict with key, values to be added to the main dictionary 此外，我们返回一个包含键值对的字典，这些键值对将添加到主字典中
    """
    ...
    if 'include_human_parts' in task_dictionary.keys() and task_dictionary['include_human_parts']:
        # Human Parts Segmentation 人体部位分割
    ...
    if 'include_depth' in task_dictionary.keys() and task_dictionary['include_depth']:
        ...
        task_cfg.FLAGVALS[tmp] = cv2.INTER_NEAREST
        task_cfg.INFER_FLAGVALS[tmp] = cv2.INTER_LINEAR
        other_args['depthloss'] = 'l1'                                    # get loss的时候有设置

    if 'include_class' in task_dictionary.keys() and task_dictionary['include_class']:
        # 分类
        assert(db_name == 'ALGAE')
        tmp = 'class'
        task_cfg.NAMES.append(tmp)
        task_cfg.NUM_OUTPUT[tmp] = 7
        task_cfg.FLAGVALS[tmp] = cv2.INTER_NEAREST
    ...
    if 'include_regres' in task_dictionary.keys() and task_dictionary['include_regres']:
        # 回归预测
        assert(db_name == 'ALGAE')
        tmp = 'regres'
        task_cfg.NAMES.append(tmp)
...
def create_config(env_file, exp_file):
    ...
    with open(env_file, 'r') as stream:
        root_dir = yaml.safe_load(stream)['root_dir']     # 加载yaml文件，读取env_file，并将其存入stream,将结构体中的"root_dir"放置root_dir
   
    with open(exp_file, 'r') as stream:
        config = yaml.safe_load(stream)
    
    
    # Copy all the arguments 复制所有参数，将配置文件的参数保存至一个方便访问的对象
    cfg = edict()                                         # 创建字典，并将键-值放进cfg中
    for k, v in config.items():
        cfg[k] = v

    
    # Parse the task dictionary separately 单独解析任务字典
    ...
    cfg.ALL_TASKS.NAMES = []                               # 所有任务的名字
    cfg.ALL_TASKS.NUM_OUTPUT = {}                          # 所有任务输出数目
    ...
    # Parse auxiliary dictionary separately 单独解析辅助字典
    ...
    elif cfg['train_db_name'] == 'ALGAE':
        cfg.TRAIN = edict()
        cfg.TRAIN.SCALE = (100, 100)
        cfg.TEST = edict()
        cfg.TEST.SCALE = (100, 100)

    else:
        raise NotImplementedError


    # Location of single-task performance dictionaries (For multi-task learning evaluation) 单任务性能字典的位置（用于多任务学习评估）：放置了单任务的性能指标
    if cfg['setup'] == 'multi_task':
        ...
        for task in cfg.TASKS.NAMES:
            ...
            test_dict = os.path.join(task_dir, 'results', '%s_test_%s.json' %(cfg['test_db_name'], task))  # val_db_name->test_db_name
            cfg.TASKS.SINGLE_TASK_TEST_DICT[task] = test_dict
            cfg.TASKS.SINGLE_TASK_VAL_DICT[task] = val_dict

    # Overfitting (Useful for debugging -> Overfit on small partition of the data) 过拟合（用于调试 -> 在数据的小部分上进行过拟合）
    if not 'overfit' in cfg.keys():
        cfg['overfit'] = False

    # Determine output directory 决定输出字典
    ...
    cfg['checkpoint'] = os.path.join(output_dir, 'exp')
    cfg['best_model'] = os.path.join(output_dir, 'best_model.pth.tar')
    cfg['pt_model'] = os.path.join(output_dir, 'model.pt')
    mkdir_if_missing(cfg['output_dir'])
    mkdir_if_missing(cfg['save_dir'])
    mkdir_if_missing(cfg['checkpoint'])
    return cfg
