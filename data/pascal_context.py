...
class PASCALContext(data.Dataset):
    ...
    VOC_CATEGORY_NAMES = ['background',                                                                        #  VOC 数据集（PASCAL VOC）中的类别名称列表
    ...
    def __init__(self,
        ...
        self.do_human_parts = do_human_parts                                                                            # 控制是否进行人体分割任务
        part_gt_dir = os.path.join(self.root, 'human_parts')                                                            # 获取数据的文件夹
        self.parts = []
        self.human_parts_category = 15                                                                                  # 说明人体部位种类为15种
        print(PROJECT_ROOT_DIR)
        self.cat_part = json.load(open(os.path.join(PROJECT_ROOT_DIR, 'data/db_info/pascal_part.json'), 'r'))           # 第15行表示人体部位名称
        self.cat_part["15"] = self.HUMAN_PART[num_human_parts]                                                          # 将用户指定的人体部位传递给cart_part的第15行
        self.parts_file = os.path.join(os.path.join(self.root, 'ImageSets', 'Parts'),                                   # 定义了存储人体部位信息的文件路径，？？？？？？？？           
                                       ''.join(self.split) + '.txt')                                                    # 2008_000008为图像编码，后面的数字为检测出个体的类别
        ...
        self.normals = []
        if self.do_normals:
            with open(os.path.join(PROJECT_ROOT_DIR, 'data/db_info/nyu_classes.json')) as f:                            # 将nyud数据集中的类别储存到cls_nyu 
                cls_nyu = json.load(f)
            with open(os.path.join(PROJECT_ROOT_DIR, 'data/db_info/context_classes.json')) as f:                        # 将pascal数据集中的类别储存到cls_context
                cls_context = json.load(f)

            self.normals_valid_classes = []                                                                             # 筛选出在 cls_nyu 列表中存在于 cls_context 中且不是 'unknown' 类别的元素，并将这些元素对应的值存储到 self.normals_valid_classes 列表中
            for cl_nyu in cls_nyu:
                if cl_nyu in cls_context and cl_nyu != 'unknown':
                    self.normals_valid_classes.append(cls_context[cl_nyu])

            # Custom additions due to incompatibilities                                                                 # 由于不兼容性的定制添加
        ...
        print("Initializing dataloader for PASCAL {} set".format(''.join(self.split)))             # 加载各类数据
        ...
        if self.do_human_parts:
            # Find images which have human parts                                                      # 找到具有人体部位的图片
            self.has_human_parts = []
            for ii in range(len(self.im_ids)):
                if self.human_parts_category in self.part_obj_dict[self.im_ids[ii]]:                  # im_ids就是imname,后面的编号出现15(self.human_parts_category)则说明有人类部位出现
                    self.has_human_parts.append(1)                                                    # 如果具有人体部位，self.has_human_parts追加1作为标识
                else:
                    self.has_human_parts.append(0)                                                    # 如果没有人体部位，self.has_human_parts追加0作为标识

            # If the other tasks are disabled, select only the images that contain human parts, to allow batching  如果其他任务被禁用，则只选择包含人体部位的图像，以便进行分批处理
            if not self.do_edge and not self.do_semseg and not self.do_sal and not self.do_normals:   # 如果只有人体部位检测，则删去不含人体的输入数据
    ...
    def __getitem__(self, index):
        ...
        _img = self._load_img(index)                                                                               # 将输入图像和标签全部输入到sample这个列表中，并保证两者尺寸相等
        sample['image'] = _img
    ...
    def _load_edge(self, index):
        # Read Target object                                                                               # 读取目标文件
        _tmp = sio.loadmat(self.edges[index])
        _edge = cv2.Laplacian(_tmp['LabelMap'], cv2.CV_64F)                                                # 利用拉普拉斯进行计算边缘信息
        _edge = thin(np.abs(_edge) > 0).astype(np.float32)
        return _edge

    def _load_human_parts(self, index):
        if self.has_human_parts[index]:                                                                    # 检查给定索引的图像是否包含人体部分

            # 读取目标对象的数据
            _part_mat = sio.loadmat(self.parts[index])['anno'][0][0][1][0]                                 #### self.parts[]表示存放所有输入数据的列表，human_part数据中有一个名为anno的数据结构体，结构体里面有两个键对1.imname:'2008_000515',2.objest
                                                                                                           # object里面有class(物种)、class_ind（物种编码）、mask（整个物种的mask）和parts，parts里面又有part_name(物种每个部位名称)和mask(物种每个部位名称mask)
            _inst_mask = _target = None

            for _obj_ii in range(len(_part_mat)):

                has_human = _part_mat[_obj_ii][1][0][0] == self.human_parts_category                       # 遍历_part_mat所有数据，检查是否有人体以及具体部分的数据
                has_parts = len(_part_mat[_obj_ii][3]) != 0                                                # 验证是否存在，并返回至has_parts

                if has_human and has_parts:                                                                # 这个可能是返回整个人的掩码
                    if _inst_mask is None:
                        _inst_mask = _part_mat[_obj_ii][2].astype(np.float32)
                        _target = np.zeros(_inst_mask.shape)
                    else:
                        _inst_mask = np.maximum(_inst_mask, _part_mat[_obj_ii][2].astype(np.float32))      # 逐像素对比两个举证，返回最大值

                    n_parts = len(_part_mat[_obj_ii][3][0])                                                # 将不同部位的掩码mask_id传递给_target[mask]，使得每一张掩码图都有对应的人体部位类别标签
            ...
            if _target is not None:                                                                        # 如果有各个部位的掩码_target和整个人的掩码_inst_mask都存在，则转换数据形式，否则用0替代
    ...
    def _get_semseg_fname(self, fname):                                                                             # 返回图像分割数据的文件框架
    ...
    def _check_preprocess_parts(self):
        _obj_list_file = self.parts_file                                                                           # 具有人体部位信息的text文件路径
        if not os.path.isfile(_obj_list_file):
            return False
        else:
            self.part_obj_dict = json.load(open(_obj_list_file, 'r'))                                              # 获取具有人体部位信息                                              

            return list(np.sort([str(x) for x in self.part_obj_dict.keys()])) == list(np.sort(self.im_ids))        # 如果序号能对上，则返回列表

    def _preprocess_parts(self):
        self.part_obj_dict = {}
        obj_counter = 0
        for ii in range(len(self.im_ids)):
            # Read object masks and get number of objects                                                         # 读取目标物的mask和目标物数目
            if ii % 100 == 0:
            ...
            # Get the categories from these objects                                                               # 得到这些目标物的种类
            _cat_ids = []
