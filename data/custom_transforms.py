...
class ScaleNRotate(object):
    """Scale (zoom-in, zoom-out) and Rotate the image and the ground truth 对图像和真实数值进行缩放（放大、缩小）和旋转.
    Args:
...
class FixedResize(object):
    """Resize the image and the ground truth to specified resolution 将图像和真实数值调整为指定的分辨率大小.
    Args:
...
class RandomHorizontalFlip(object):
    """Horizontally flip the given image and ground truth randomly with a probability of 0.5. 以0.5的概率随机对给定的图像和真实数值进行水平翻转"""
...
class RandomErasing(object):
    """Randomly erase pixels of the given image with a probability of 0.25. 以0.25的概率随机擦除给定图像的像素点"""
        ...
        if random.random() < 0.25:
            for elem in sample.keys():
                if 'meta' in elem:
                    ...
                    # 随机选择擦除区域的起始点
                    x = random.randint(0, width - 10)  # 修正擦除区域起始点的取值范围
                    y = random.randint(0, height - 10)  # 修正擦除区域起始点的取值范围
                    # 随机选择擦除区域的宽度和高度
                    erase_width = random.randint(10, width - x)
                    erase_height = random.randint(10, height - y)
                    # 将擦除区域像素值设为0
                    tmp[y:y+erase_height, x:x+erase_width] = 0
                    sample[elem] = tmp
        ...
        if random.random() < 0.5:
            for elem in sample.keys():
                if 'meta' in elem:
                    ...
                    tmp = sample[elem]
                    tmp = cv2.flip(tmp, flipCode=0)  # flip vertically
                    sample[elem] = tmp
                if elem == 'normals':
                    sample[elem][:, :, 1] *= -1  # flip y-component of normals
        return sample
...
class RandomRotation(object):
    """Randomly rotate the given image and ground truth by a random multiple of 90 degrees."""

    def __call__(self, sample):
        angle = random.randint(0, 3) * 90  # Randomly select 0, 90, 180, or 270 degrees
        for elem in sample.keys():
            if 'meta' in elem:
                continue
            else:
                ...
                sample[elem] = tmp
            if elem == 'normals':
                # Rotate normals by the same angle
                theta = np.radians(angle)
                R = np.array([[np.cos(theta), -np.sin(theta)],
                              [np.sin(theta), np.cos(theta)]])
                sample[elem] = np.dot(sample[elem], R.T)
        return sample
...
class RandomBrightnessContrast(object):
    """Randomly adjust the brightness and contrast of the given image."""

    def __init__(self, brightness_range=(-0.2, 0.2), contrast_range=(-0.2, 0.2)):
        self.brightness_range = brightness_range
        self.contrast_range = contrast_range

    def __call__(self, sample):
        alpha = 1.0 + random.uniform(self.contrast_range[0], self.contrast_range[1])
        beta = random.uniform(self.brightness_range[0], self.brightness_range[1])
        for elem in sample.keys():
            if 'meta' in elem:
                continue
            else:
                ...
                sample[elem] = np.clip(tmp, 0, 255).astype(np.uint8)
        return sample
...
class ToImage(object):
    """
    Return the given elements between 0 and 255 返回给定的元素，取值范围在0到255之间
    """
...
class NormalizeImage(object):
    """
    Return the given elements between 0 and 1 将给定的元素返回在0和1之间
    """
...
class NormalizeImage(object):
    """
    Normalize the given elements between 0 and 1 based on per-layer mean and standard deviation.
    """
    def __init__(self, norm_elem='image', 
                 mean_values = [0.8812, 0.8846, 0.8846, 0.8841, 0.8854, 0.8254, 0.8153, 0.8082, 0.8056, 0.8014, 0.8323, 0.8280, 0.8252, 0.8201, 0.8147, 0.7540, 0.7512, 0.7563, 0.7537, 0.7527], 
                 std_values = [0.0548, 0.0546, 0.0545, 0.0548, 0.0551, 0.0599, 0.0616, 0.0640, 0.0656, 0.0660, 0.0693, 0.0691, 0.0688, 0.0685, 0.0684, 0.0537, 0.0539, 0.0543, 0.0547, 0.0550], 
                 clip=False):
        self.norm_elem = norm_elem
        ...
        if isinstance(self.norm_elem, tuple):
            for elem in self.norm_elem:
                for i in range(elem.shape[0]):
                    mean = self.mean_values[i]
                    std = self.std_values[i]
                    elem = sample[i]
                    if self.clip:
                       elem = np.clip(elem, 0, 1)
                    elem = (elem - mean) / std
                    sample[i] = elem
        else:
            if self.clip:
                sample = np.clip(sample, 0, 1)
            sample = (sample - self.mean_values) / self.std_values
...
class ToImage20(object):
    ...
    def __init__(self, norm_elem='image',
        ...
        # self.mean_values = mean_values
        # self.std_values = std_values
        self.mean_values = np.load(r'D:\MTL\MTL\mean_values.npy')
        self.std_values = np.load(r'D:\MTL\MTL\std_values.npy')
...
class ToTensor(object):
    """Convert ndarrays in sample to Tensors. 将样本中的ndarrays转换为张量(Tensors)"""
    def __init__(self):
